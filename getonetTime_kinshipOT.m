%% Scirpted by Haiyan Wu(haiyanwu3@gmail.com) for extracting onsetTimes for stimuli with four different conditions in oxytocin fMRI experiment e.g., sc=1; oc=2; sa=3; oa=4 
%  2016-10-25 
...
predef_names={}; 
predef_names{end+1} = 'selfchild';
predef_names{end+1} = 'otherchild';
predef_names{end+1} = 'selfadult';
predef_names{end+1} = 'otheradult';
...
for iSubject=1:nSubjects
    ...
    for iSession =1:nSessions
        ...
        outfile = sprintf('conditions_%d_%d.mat',subject,session);  % tip: use something like %05d to print several leading zero's
        % add path in front of filename
        if isempty(outputdir)
            outfile = fullfile(fileparts(csvfile),outfile);
        else
